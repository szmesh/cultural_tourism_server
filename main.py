...
import api
# from api import verify, login, welcome
# from api import login
# from api import welcome
...
session_opts = {
    ...
    'session.data_dir': '/Users/a44116141/webapp/nginx/sessions/data',
    'session.auto': True
}
...
def validate():
    ...
    if path_info in ['/mcts/favicon.ico', '/', '/mcts/', '/mcts/api/verify/']:
        return
    ...
    url_list = ["/mcts/api/login/", "/mcts/api/logout/"]
...
def kill_port(port):
    command = '''kill -9 $(lsof -i:''' + str(port) + '''| awk '{print $2}' | grep -v 'PID')'''
    os.system(command)
